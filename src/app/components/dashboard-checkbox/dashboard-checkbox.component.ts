...
export class DashboardCheckboxComponent implements OnInit, OnDestroy, ControlValueAccessor {
  ...
  @Input() set selects(val: { value: any; label: string; }[]) {
    this.fa.clear({ emitEvent: false });
    val.forEach(v => {
      this.fa.push(new FormControl((this.fc.value as number[]).some(v2 => v2 === parseInt(v.value, 10))));
    });
  ...
  ngOnInit(): void {
    this.s.add(
      this.fa.valueChanges.subscribe((res: boolean[]) => {
        const result = res
          .map((v, i) => v ? this.selects[i].value : null)
          .filter(v => v !== null);
